...
function dist(x1, y1, x2, y2) {
  return wasm_dist(x1, y1, x2, y2);
}

// Safe wrapper for WASM collision checks that handles Result types
...
// Safe wrapper for WASM magnet zone checks that handles Result types
...
function applyGameText() {
  ...
  const h = (id, txt) => {
    const el = document.getElementById(id);
    if (el && txt !== undefined) el.innerHTML = txt;
  };
...
class Gem {
  ...
  update(dt) {
    ...
    if (
      dist(this.x, this.y, mainPlayer.x, mainPlayer.y) <
      mainPlayer.radius + this.radius + 10
    ) {
      this.dead = true;
      gemsCount++;
    ...
    if (this.exploded) {
      ...
      if (this.radius >= this.maxRadius) {
        ...
        for (let enemy of enemies) {
          if (
            !enemy.dead &&
            !this.enemiesHit.includes(enemy) &&
            dist(this.x, this.y, enemy.x, enemy.y) < this.radius + enemy.radius
          ) {
            enemy.takeDamage();
  ...
  draw() {
    ...
    if (playerUpgrades.aura > 0 && shieldCharges > 0) {
      ...
      for (let i = 0; i < shieldCharges; i++) {
        const angle =
          ((Math.PI * 2) / shieldCharges) * i + performance.now() / 500;
...
class EnemyBase {
  constructor(radius, speedBase, imgRef) {
    ...
    this.x = Math.max(
      this.radius + 10,
      Math.min(MAP_WIDTH - this.radius - 10, this.x),
    );
    this.y = Math.max(
      this.radius + 10,
      Math.min(MAP_HEIGHT - this.radius - 10, this.y),
    );
...
class Projectile {
  ...
  update(dt) {
    const currentSpeed =
      this.baseSpeed * (1 + playerUpgrades.bulletSpeed * 0.4);
    ...
    for (let enemy of enemies) {
      if (!enemy.dead && !this.pierceHits.includes(enemy)) {
        if (
          dist(this.x, this.y, enemy.x, enemy.y) <
...
function endGame() {
  ...
  if (pbScoreDisplay)
    pbScoreDisplay.innerText = localStorage.getItem("pbScore");
  ...
  if (reviveContainer)
    reviveContainer.style.display = canRevive ? "block" : "none";
  ...
  if (restartBtn) {
    ...
    const timerInt = setInterval(() => {
      ...
      if (countdown <= 0) {
        ...
        if (restartTimerDisplay)
          restartTimerDisplay.innerText = `(${countdown}s)`;
      }
    }, 1000);
  }
...
function autoBlaster() {
  ...
  if (playerUpgrades.hawk > 0) {
    ...
    if (sortedEnemies.length > 0) {
      angleToFire = Math.atan2(
        sortedEnemies[0].enemy.y - mainPlayer.y,
        sortedEnemies[0].enemy.x - mainPlayer.x,
      );
    } else {
      angleToFire = Math.atan2(
        camera.y + mouse.y - mainPlayer.y,
        camera.x + mouse.x - mainPlayer.x,
      );
    }
...
function update(dt) {
  ...
  if (frameDeltaAccumulator >= 1.0) {
    ...
    if (
      blasterCooldown <= 0 &&
      (playerUpgrades.blaster > 0 || playerUpgrades.hawk > 0)
    ) {
      autoBlaster();
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    if (
      shieldCharges > 0 &&
      distToPlayer < mainPlayer.radius + enemy.radius + 40
    ) {
      enemy.takeDamage();
      shieldCharges--;
      continue;
    ...
    if (
      invincibleTimer > 0 &&
      distToPlayer < mainPlayer.radius + enemy.radius
    ) {
      enemy.takeDamage();
      continue;
    }
    ...
    if (postLevelUpGrace <= 0 && invincibleTimer <= 0) {
      ...
      if (enemy.eatCooldown <= 0) {
        for (let j = followers.length - 1; j >= 0; j--) {
          ...
          if (
            dist(follower.x, follower.y, enemy.x, enemy.y) <
            follower.radius + enemy.radius
          ) {
            screenShake = 15;
  ...
  if (!specialWaveActive) {
    ...
    if (
      enemies.length < targetEnemyCount &&
      Math.random() < Math.max(0.1, dt)
    ) {
      spawnEnemy();
    }
  }
  ...
  for (let i = items.length - 1; i >= 0; i--) {
    ...
    if (
      dist(mainPlayer.x, mainPlayer.y, item.x, item.y) <
      mainPlayer.radius + item.radius + 15
    ) {
      items.splice(i, 1);
      addXp();
      score++;
  ...
  for (let i = powerups.length - 1; i >= 0; i--) {
    const p = powerups[i];
    if (
      dist(mainPlayer.x, mainPlayer.y, p.x, p.y) <
      mainPlayer.radius + p.radius
    ) {
      if (p.type === "nuke") {
...
function draw(time) {
  ...
  if (
    ...
    if (invincibleTimer > 0) {
      ...
      ctx.shadowColor = "#000";
      ctx.fillText(
        `⭐ ${Math.ceil(invincibleTimer)}`,
        mainPlayer.x,
        mainPlayer.y - mainPlayer.radius - 20,
      );
    }
